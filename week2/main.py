# 1. Write a function M axAvColumn(A) whose input is a matrix A. The
...
def MaxAvColumn(A: list[list]) -> int:
...
def AllIndicies(A: list) -> bool:
  n = len(A)

  allIndicies = list(range(n))

  for e in A:
    # check if the element in the allIndicies list
    if e in allIndicies:
      # check if the element appears more than once in the allIndicies list
      if allIndicies.count(e) > 1:
        return False
      else:  
        allIndicies.pop(allIndicies.index(e))

  return len(allIndicies)  == 0
...
# should return T rue if A is a Latin square and F alse otherwise. Hint: use
# the function AllIndices as in the previous question.

def isLatinSquare(A: list[list]) -> bool:
  n = len(A)
  m = len(A[0])
...
def journeyLength(A: list[list], S:list):
  # A: Distances matrix
...
def closeCities(distances: list[list]) -> int:
  ...
  for i in range(n-1):
    for j in range(i+1, n):
      # check if two cities are not same city
      if i != j:
